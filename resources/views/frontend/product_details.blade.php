            ...
            <div class="bg-white py-3">
                <div class="row">
                    ...
                    <div class="col-xl-5 col-lg-5 text-start">
                        @php
                            $couponDetails = [];
                            if (isset($globalCoupon) && $globalCoupon->type === 'cart_base' && $globalCoupon->status == 1) {
                                $couponDetails = json_decode($globalCoupon->details, true) ?? [];
                                $discount = $globalCoupon->discount;
                                $discountType = $globalCoupon->discount_type;
                                $endDate = \Carbon\Carbon::createFromTimestamp($globalCoupon->end_date)->format('F d, Y');
                                $remainingTime = \Carbon\Carbon::now()->diff(\Carbon\Carbon::createFromTimestamp($globalCoupon->end_date));
                            }
                        @endphp
                       ...
                       @if (!empty($couponDetails))
                        ...
                        @if ($isMultiple == 'yes')
                            ...
                            <div class="card-body p-1">
                                <div class="attribute-selection ">
                                    <p class="property_name text-dark" data-property-name="{{ $propertyNames['first_property'] }}"
                                ...
                                <div class="d-flex flex-wrap align-items-center">
                                    <table class="table">
                                        <thead>
                                            <tr>
                                                <th class="attribute-name">{{ ucwords($propertyNames['second_property']) }}
                                                </th>
                                                <th>Price</th>
                                                <th>Stock</th>
                                                <th>Quantity</th>
                                                    ...
                                                    <tr class="configured-item-row"
                                                        ...
                                                        <td>
                                                            <div class="input-group quantity-group">
                                                                <button class="btn btn-outline-secondary decrement-btn px-2"
                                                                    type="button">-</button>
                                                                <input type="text"
                                                                    class="form-control text-center quantity-input"
                                                                    value="0" style=" padding: 0;">
                                                                <button class="btn btn-outline-secondary increment-btn px-2"
                                                                    type="button">+</button>
                                                            </div>
                                                        </td>
                                                    </tr>
                        ...
                        <div class="">
                            ...
                            <h6>Approximate Weight: <span
                                    class="total_weight">{{ $detailedProduct['ActualWeightInfo']['Weight'] }} </span> KG (আনুমানিক)
                            </h6>
                            <span class="text-danger">* বিঃ দ্রঃ প্রদত্ত ওজন ১০০ ভাগ সঠিক নয়। বাংলাদেশ ওয়ারহাউজে পণ্য রিসিভ হওয়ার পর সঠিক ওজন জানিয়ে দেওয়া হবে ।</span>
                        </div>
                    ...
                    <div class="col-xl-3 col-lg-3">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="text-danger"><small>China courier charge + China to Bangladesh shipping charge, will be added later.</small></p> 
                        ...
                        <p class="text-danger">**After the product arrives in Bangladesh, the final shipping charge will be determined based on
                    ...
                    <p class="mb-1"><strong>Approximate Shipping Cost: ৳</strong><span
                            ...
                            <div class="form-row mt-4">
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control selectpicker" id="category" data-live-search="true" onchange="categoryInfo()">
...
document.querySelectorAll('.single-property-option').forEach(function(option) {
    ...
    propertyOptions.forEach(option => {
        option.addEventListener("click", function () {
            resetCarousel();
            let newImageUrl = this.getAttribute("data-large-image-url"); // Get new image from property option
    ...
    thumbnails.forEach(thumb => {
        ...
        propertyOptions.forEach(option => {
            option.addEventListener("click", function () {
                resetCarousel();
                let newImageUrl = this.getAttribute("src"); // Get new image from property option
...
function showMatchingRows(matchingItems, itemPropertyValue, itemselectedImage) {
    ...
    matchingItems.forEach((item, index) => {
        if (item.Id) {  // Ensure Id exists
            item.Configurators.forEach(configurator => {
                let matchingRow = Array.from(document.querySelectorAll('.configured-item-row')).find(row => {
            ...
            function updateTotalQuantity() {
                ...
                if (minQuantity > 0) {
                    ...
                    if (totalQuantity >= minQuantity) {
                        addToCartBtn.disabled = false; // Enable if total quantity is greater than or equal to minQuantity
                    } else {
