...
<div class="aiz-card-box h-auto bg-white hov-scale-img" style="border: 1px solid #e7e7e7;">
    <div class="position-relative h-170px h-sm-170px h-md-200px img-fit overflow-hidden">
        <!-- Image -->
...
@endphp
               ...
               <div class="d-flex flex-column">
                    <div>
                        ...
                        @if (isset($discountedPrice) && $discountedPrice < $originalPrice)
                            <span class="text-muted ms-2 fs-6"><del>৳ {{ number_format($originalPrice, 2) }}</del></span>
    ...
    <div class="card-footer d-none d-md-flex justify-content-between align-items-center p-2">
        <!-- Seller Name -->
        <div class="seller-name">
            <span class="text-muted fs-md-12 fs-sm-20">{{ \Illuminate\Support\Str::limit($product['VendorName'] ?? '', 15) }}</span>
        </div>

        <!-- View Seller Button -->
        @if (!empty($product['VendorId']) && !empty($product['VendorName']))
            <div class="view-seller">
                <a href="{{ route('shop.visit', ['vendorId' => $product['VendorId'], 'vendorName' => $product['VendorName']]) }}" target="_blank"
                   class="btn btn-sm btn-danger" style="padding: 0.216rem .5rem;">
                    View Seller
                </a>
            </div>
        @endif
